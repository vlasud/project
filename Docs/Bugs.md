...
**Зато настоящая дыра того же класса нашлась рядом — в `RoleplayChatSystem` (исправлено).** Там была своя половинчатая чистка `sanitizeEmote`: снимала `{`/`}` и управляющие байты, но ПРОПУСКАЛА `~`. Замер: `/me tilde ~r~ here` доезжал до чужих клиентов как `* Lo_Vlasud tilde ~r~ here`. Фикс — прогонять текст через штатный `Encoding::neutralizeLine` (он снимает `{}`, `~`, управляющие и `0x7F`); после правки клиент получает `* Lo_Vlasud tilde -r- here`. Мораль для ревью: своя копия общей чистки отстаёт от оригинала — искать именно такие половинки.
