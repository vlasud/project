...
### 11. `ChatSystem` рассылает сырой текст чата без чистки цветовых кодов
`ChatSystem.cpp:95-156` · `cheat-abuse`
`onPlayerText` встраивает сырое клиентское сообщение в формат-строку без `neutralizeColorCodes` → игрок вставляет `{RRGGBB}`/`~…~`, ломая цвета/раскраску у всех в радиусе. *(нарушает memory chat-color-code-injection.)*
**Фикс:** прогнать `message` через `Encoding::neutralizeColorCodes` перед форматированием (зеркально `/me,/do,/try`).
