...
`Src/Systems/VehicleLockSystem`.

> **ВХОДА В ФИЧУ СЕЙЧАС НЕТ.** Тумблер «Закрыть двери» / «Открыть двери» убран из
> `/car` вместе с остальным управлением состоянием машины (`Docs/CarMenu.md`), а
> другого способа переключить замок игроку не дано. Сервис и система живы и
> корректны: состояние переприменяется на стрим-ине, чистится на уничтожении
> экземпляра и на флипе шеринга — но `toggle` никто не зовёт, поэтому фактически
> все машины открыты. Дальше замку нужен новый вход (клавиша за рулём или
> команда) — отдельной задачей; всё описанное ниже к этому моменту готово.
...
## API для будущего входа

- **`isLocked(vehicleId)`** — состояние для лейбла/статуса.
- **`toggle(vehicleId)`** — флип + немедленное применение; возвращает НОВОЕ
  состояние (для текста сообщения об итоге). **Гейт владения — на стороне
  вызывающего:** прежний вход (`CarMenuSystem`) проверял, что машина в `owned()`
  игрока, и ре-валидировал индекс на клике; новый вход обязан сделать то же (или
  взять машину, за рулём которой игрок сидит). Сервис дополнительно
  bounds/exists-safe (`setLocked` — no-op для несуществующей машины, страховка на
  случай гонки уничтожения).

## Три правила

- **Читер-абуз:** переключать замок сейчас НЕКОМУ (вход убран) — а новый вход
  обязан гейтить владение так же, как прежний: машина ТОЛЬКО из списка владения
  игрока (`owned(playerId)` с ре-валидацией на каждом клике) — чужую или
  расшаренную семье ЧУЖОГО владельца машину закрывать/открывать нельзя (её нет в
  его `owned`). Дистанционность («брелок») прав не расширяет: множество
  доступных машин то же. Политика доступа
...
- **Перф:** `isLocked`/`setLocked` (без переприменения) — O(1) hash-lookup;
  `reapplyToStreamed` — O(MAX_PLAYERS), но ТОЛЬКО на редких событиях (флип замка,
  share/unshare) — не per-tick и не per-stream-in-конкретного-игрока (тот
