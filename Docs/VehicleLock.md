...
- **`toggle(vehicleId)`** — флип + немедленное применение; возвращает НОВОЕ
  ...
  СВОЕЙ машины — гейт владения в `CarMenuSystem` (машина по `carIndex` из
  `owned()` игрока, ре-валидация на клике) ДО вызова. Посадка НЕ требуется —
  замок работает как «брелок» на любом расстоянии (нужен только живой экземпляр);
  сервис дополнительно bounds/exists-safe (`setLocked` — no-op для
  несуществующей машины, страховка на случай гонки уничтожения между кликами).

## Три правила

- **Читер-абуз:** замок читает/переключает ТОЛЬКО `CarMenuSystem` по машине из
  СПИСКА ВЛАДЕНИЯ игрока (`carIndex` -> `owned(playerId)`, ре-валидация на каждом
  клике) — чужую или расшаренную семье ЧУЖОГО владельца машину не закрыть/открыть
  (её нет в его `owned`). Дистанционность («брелок») прав не расширяет: множество
  доступных машин то же, что и раньше. Политика доступа
