...
std::int64_t BusWalletService::withdraw(int playerId, AccountId accountId)
{
    ...
    // SET=0: пул БД (конкурентные воркеры) не гарантирует порядок async-записей.
    // Относительные -amount (withdraw) и +amount (add) КОММУТИРУЮТ, поэтому итог
    // сходится к верному независимо от порядка; абсолютный SET=0 мог бы обогнать
    // ещё не применённый +amount и дать рассинхрон/дюп на релоге. Чистый UPDATE
    // (не UPSERT): строка всегда существует (withdraw гейтится balance>0), нет
    // строки — безопасный no-op (списывать нечего).
    DatabaseManager::throwQuery(
        [accountId, amount](mysqlx::Schema schema)
        {
            schema.getSession()
                .sql("UPDATE bus_wallet SET balance = balance - ? WHERE account_id = ?")
