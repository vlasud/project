...
class MapIconService final : public IService
{
  ...
  public:
    ...
    // Дистанция «иконка ровно в радаре игрока»: радиус мира, который покрывает
    // радар. Стримить дальше бессмысленно и вредно — стиль MapIconStyle_Global
    // прижимает недостижимую иконку к КРАЮ радара, и она указывает на то, чего в
    // видимой области ещё нет. С этой дистанцией иконка приезжает ровно тогда,
    // когда точка входит в радар, и края не занимает.
    //
    // Значение — покрытие радара на земле; если иконка появляется заметно раньше
    // (висит на краю) или позже (возникает уже внутри), правится ЗДЕСЬ, одним
    // числом на всех потребителей. Подобрано вживую: 300 (дефолт стримера) и 200
    // давали иконку задолго до входа точки в радар.
    static constexpr float RADAR_STREAM_DISTANCE = 150.0f;
