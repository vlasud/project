...
class PlayerAnimationService final : public IService
{
  public:
    ...
    // Предзагрузить библиотеку анимаций на клиент игрока. Первый ApplyAnimation из
    // ещё НЕ загруженной либы клиент не проигрывает (только начинает стримить её
    // IFP-блок) — вызвать заранее (например, при старте активности), чтобы первая же
    // реальная анимация из либы проигралась сразу. Ничего видимого не проигрывает и
    // НЕ трогает серверное состояние анимации (анти-чит предзагрузку игнорирует).
    void preloadLibrary(IPlayer &player, StringView lib);
