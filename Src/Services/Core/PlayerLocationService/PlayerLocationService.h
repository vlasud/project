...
// клиентской смены интерьера с переносом позиции не бывает; серверные переносы
// идут через teleport()/setInterior() (грейс позиции — из pendingTeleport).
// Поэтому клиентский onInteriorChange НЕ даёт грейса позиции (иначе чит спамом
// интерьеров 1/0/1/0 легализовал бы телепорт) — он лишь обновляет принятый
// интерьер; позиция всегда проходит обычную проверку достижимости.
//
