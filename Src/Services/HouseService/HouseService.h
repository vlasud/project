...
// Дома — player houses (бизнес-фича, НЕ Core). Источник правды о домах + персист
// в JSON-файл houses.json рядом с сервером (рабочая директория, не БД). Дом —
// пикап ВХОДА в позиции создателя + ИНТЕРЬЕР из каталога готовых интерьеров;
// внутри — пикап ВЫХОДА. id дома — порядковый номер создания.
//
// Дома пока ВСЕГДА ничейные (зелёная иконка): поле владельца есть и красная
// ветка готова под будущую покупку/назначение, но сейчас owner всегда пуст.
//
...
class HouseService final : public IService
{
  ...
  public:
    ...
    struct House
    {
        ...
        std::string owner;       // utf-8; "" — ничейный (зелёный)
    };
    ...
    // --- сериализация JSON (фиксированная схема домов) ---
