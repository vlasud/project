...
// (HomeMenuSystem через HouseService::setOwner) идут через
...
class HouseSystem : public BaseSystem
{
  ...
  private:
    ...
    void createHouseFor(IPlayer &player, int interiorIndex, int parkingCap, std::int64_t price);
    ...
    // markOwnershipLoaded (флаг готовности + one-shot оповещение подписчиков, напр.
    // SpawnChoiceSystem) зовётся ТОЛЬКО на успехе. На ошибке БД флаг остаётся снятым:
    // им гейтятся итоги аукционов домов, а с пустым зеркалом владения раздача ушла бы
    // не тому и необратимо стёрла бы чужую строку house_owner. Цена — до перезапуска
    // спавн «Дом» резолвится фолбэком, а торги домов стоят (ставки и деньги целы).
