...
void ParkingSystem::onParkingPickup(IPlayer &player)
{
    ...
    m_dialogService.show(
        ...
        [this, playerId](DialogResponse response, int listItem, StringView)
        {
            ...
            // парковка НЕ спавнит: у неё своё место у дома, куда её подаёт ВЫЗОВ
            // из /car. Спавн отсюда либо вернул бы дубль (машина уже вызвана),
            // либо увёл бы машину с её точки.
            if (m_parkedService.isParked(owned[listItem].dbId))
            {
                // Текст честен для всех состояний припаркованной — «в гараже»
                // (не вызвана), «у дома» и «брошена» (уехал и вышел): слово
                // отказа не должно противоречить статусу строки списка.
                player->sendClientMessage(
                    ERROR_COLOUR,
                    u("Эта машина припаркована у дома — вызвать её или найти можно через /car"));
                return;
            }
