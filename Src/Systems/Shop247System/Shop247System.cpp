...
//
// ПОРЯДОК ЗАПИСЕЙ = индекс интерьера, который лежит в businesses.json у каждой
// созданной точки. Дописывать в КОНЕЦ безопасно; вставка в середину или удаление
// сдвинут индексы и переселят существующие бизнесы в чужие комнаты — такую правку
// делают только вместе с миграцией самого json.
std::vector<BusinessService::CatalogEntry> Shop247System::interiors()
{
    return {
        ...
        // «24/7 2»: прилавок НЕ замерен — прежний замер остался от другой комнаты
        // (интерьер сменили) и лежал в тридцати метрах от спавна. Пока его нет,
        // витрина в этой комнате не открывается; выход считается по правилу.
        {"24/7 2", 4, Vector3(-27.8440f, -26.6737f, 1003.5573f), 0.0f},
        {"24/7 3", 16, Vector3(-26.1856f, -140.9164f, 1003.5469f), 0.0f},
        {"24/7 4", 6, Vector3(-26.8339f, -55.5846f, 1003.5469f), 0.0f},
        {"24/7 5", 10, Vector3(6.0856f, -28.8966f, 1003.5494f), 0.0f},
        {"24/7 6", 18, Vector3(-30.9875f, -89.6806f, 1003.5469f), 0.0f},
    };
}
