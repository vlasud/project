...
void ThreadPool::shutdown()
{
    ...
    // Завершимость: у DatabaseManager два источника таких «дозадач» — очередь
    // backpressure (releaseAndPump) и очереди упорядоченных по ключу запросов
    // (releaseKey). Каждый выпускает не больше ОДНОЙ задачи на каждую завершённую,
    // поэтому суммарное число ожидающих строго убывает и цепочка конечна
    // (ограничена глубиной обеих очередей на момент останова).
    m_draining = true;
    for (;;)
    {
        runQueuedTasksSync();
